...
def validate_synthetic_data(original_df, augmented_df, continuous_features, categorical_features, 
    ...
    # Define extract_synthetic_samples function inline to ensure it's available
    def extract_synthetic_samples(original_df, augmented_df, synthetic_column='is_synthetic'):
        """
        Extracts synthetic samples from augmented dataset using a column that indicates synthetic status.
        """
        # If the synthetic indicator column exists in augmented data, use it
    ...
    if plot:
        ...
        # Import or define visualization functions
        try:
            # Try to import functions if they've been defined externally
            from improved_visualizations import plot_feature_distributions, plot_2d_feature_comparison
        except ImportError:
            # If not available, define them inline (simplified versions)
