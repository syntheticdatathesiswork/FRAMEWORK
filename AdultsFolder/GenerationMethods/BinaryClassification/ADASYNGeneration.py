...
def augment_dataframe_adasyn(df, target, test_size=0.25, random_state=42, ratio_limit=1.0):
    """
    ...
    # Apply ADASYN with a custom sampling strategy
    adasyn = ADASYN(sampling_strategy={minority_class: desired_minority_count}, random_state=random_state)
    X_res, y_res = adasyn.fit_resample(X_train_scaled, y_train)
    
    # In ADASYN, original samples are assumed to be the first len(X_train_scaled) rows.
    ...
    if synthetic_generated > synthetic_needed:
        ...
        # Filter synthetic samples of the minority class and trim.
...
if __name__ == '__main__':
    ...
    orig_train, aug_train, test_set = augment_dataframe_adasyn(df, target='class', test_size=0.3, random_state=42, ratio_limit=1.0)
